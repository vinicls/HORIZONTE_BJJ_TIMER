// ===== HORIZONTE JIU JITSU • TIMER — v1.9.0 =====
// Mantém toda a lógica anterior + ajustes mobile/tablet + splash lock de rolagem.
// Inclui fix garantido: preparação SEMPRE antes do 1º round se configurada (5s/10s).
...
(() => {
  ...
  if (buildInfo) {
    ...
    buildInfo.textContent = `v1.9.0 • atualizado em ${dd}/${mm}/${yyyy} • desenvolvido por Vinicius Simões`;
  }
  ...
  function play(name){ 
    const on = (getActiveSoundSelect().value === 'on');
    if(!on) return;
  ...
  // Dois seletores (topo e mobile) sincronizados
  ...
  // Flag para garantir preparação inicial (somente antes do 1º round)
  ...
  function start(){
    ...
    // Sincroniza com o que está na UI
    syncCfgFromSelects();

    // Reset de contadores e estado de rodada
    lastTick = null; lastWhole = null;
    currentRound = 1;

    // Define se haverá preparação inicial
    initialPrepPending = cfg.prepSeconds > 0;

    // Entra no modo foco e seleciona a fase inicial
    enterFocus();
